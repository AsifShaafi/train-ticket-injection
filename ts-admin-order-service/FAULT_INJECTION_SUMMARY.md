...
- **Endpoint:** `POST /api/v1/adminorderservice/adminorder` AND `PUT /api/v1/adminorderservice/adminorder`
- **Triggers:** contactsName is null, empty, or purely numeric
- **Test:** Send create/update with `contactsName: "12345"`

### 2. INVALID_SEAT_NUMBER_FAULT
- **Endpoint:** `POST /api/v1/adminorderservice/adminorder` AND `PUT /api/v1/adminorderservice/adminorder`
- **Triggers:** seatNumber doesn't match pattern `^\d+[A-Z]$`
- **Test:** Send create/update with `seatNumber: "5a"`, `"A5"`, or `"123456789012345678901234567890"`
